...
>
    <template x-for="toast in toasts" :key="toast.id">
        <div 
            ...
            class="pointer-events-auto relative overflow-hidden rounded-2xl p-4 bg-slate-900/90 text-white shadow-2xl backdrop-blur-xl border border-slate-700/60 transition-all duration-300"
        >
            <div class="flex items-start gap-3.5">
                ...
                <div class="flex-shrink-0 mt-0.5">
                    ...
                    <template x-if="toast.type === 'success'">
                        <div class="w-8 h-8 rounded-xl bg-emerald-500/20 text-emerald-400 border border-emerald-500/30 flex items-center justify-center shadow-[0_0_15px_rgba(16,185,129,0.3)]">
                            <svg class="w-4 h-4 stroke-current" fill="none" viewBox="0 0 24 24" stroke-width="2.5" stroke="currentColor">
                ...
                <div class="flex-1 pr-2">
                    <h4 x-text="toast.title" class="text-sm font-bold tracking-tight text-white"></h4>
                    <p x-text="toast.message" class="text-xs font-medium text-slate-300 mt-0.5 leading-relaxed"></p>
                </div>

                <!-- Botón Cerrar -->
                <button 
                    @click="removeToast(toast.id)"
                    class="text-slate-400 hover:text-white p-1 transition-colors rounded-lg hover:bg-slate-800"
                >
