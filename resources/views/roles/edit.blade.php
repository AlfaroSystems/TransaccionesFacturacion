...
<div class="animate-fade-in duration-300 max-w-4xl mx-auto">
    ...
    <header class="mb-8 flex items-center gap-4">
        ...
        <div>
            <h1 class="text-2xl md:text-3xl font-extrabold text-navy-800 tracking-tight">Editar Rol: <span class="capitalize">{{ $role->name }}</span></h1>
            <p class="text-slate-400 text-sm font-semibold mt-1">Actualiza los datos del rol y las capacidades asignadas en el sistema.</p>
        </div>
    </header>
        ...
        <form action="{{ route('roles.update', $role) }}" method="POST">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
                <div class="w-full">
                    ...
                    <input type="text" name="name" id="name" value="{{ old('name', $role->name) }}" class="w-full bg-slate-50 border @error('name') border-rose-500 @else border-slate-200 @enderror rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:border-navy-sidebar focus:bg-white transition-all text-slate-700 font-semibold disabled:opacity-50 disabled:cursor-not-allowed" required {{ $role->name === 'admin' ? 'disabled' : '' }}>
                    @if($role->name === 'admin')
                        <!-- Input oculto para que envíe el valor del rol admin principal -->
                        <input type="hidden" name="name" value="admin">
                        <span class="text-[10px] text-slate-400 font-bold mt-1.5 block">El nombre del rol base "admin" no se puede modificar.</span>
                    @endif
                    @error('name')
                ...
                <div class="w-full">
                    ...
                    <input type="text" name="description" id="description" value="{{ old('description', $role->description) }}" placeholder="Ej: Permite gestionar artículos e inventarios" class="w-full bg-slate-50 border @error('description') border-rose-500 @else border-slate-200 @enderror rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:border-navy-sidebar focus:bg-white transition-all text-slate-700 font-semibold">
                    @error('description')
                        <span class="text-xs text-rose-500 font-bold mt-1.5 block">{{ $message }}</span>
                    @enderror
                </div>
                        ...
                        <div class="space-y-3">
                            @foreach($permissions->filter(fn($p) => str_starts_with($p->id, 'usuarios.')) as $permission)
                                <label class="flex items-start gap-3 cursor-pointer group">
                                    <input type="checkbox" name="permissions[]" value="{{ $permission->id }}" {{ in_array($permission->id, $rolePermissions) ? 'checked' : '' }} class="permission-checkbox mt-1 rounded text-navy-sidebar focus:ring-navy-sidebar border-slate-300 w-4 h-4 cursor-pointer">
                                    <div>
