        ...
        <div class="w-full md:w-3/5 p-8 flex flex-col justify-between">
            ...
            <div class="w-full max-w-sm mx-auto my-auto py-4">
                ...
                <div class="flex border-b border-gray-100 dark:border-slate-700 mb-8">
                    ...
                    <button onclick="window.location.href='{{ route('password.request') }}'" class="flex-1 text-center pb-3 text-sm font-semibold text-gray-400 dark:text-slate-400 border-b-2 border-transparent hover:text-gray-600 dark:hover:text-slate-200 transition-all">
                ...
                <form method="POST" action="{{ route('login') }}" class="space-y-5">
                    ...
                    <div class="flex items-center ml-4">
                        <input id="remember_me" type="checkbox" class="rounded border-gray-300 dark:border-slate-600 dark:bg-slate-900 text-[#3cb0a4] shadow-sm focus:ring-[#4ebbb0]" name="remember">
                        <label for="remember_me" class="ms-2 text-xs text-gray-500 dark:text-slate-400 font-medium cursor-pointer">Recordarme</label>
                    </div>

                    <!-- Footer Actions -->
                    <div class="flex items-center justify-between pt-2">
                        @if (Route::has('password.request'))
                            <a class="text-xs text-[#2b7f76] dark:text-customTeal-400 hover:text-[#3cb0a4] hover:underline font-bold transition-all" href="{{ route('password.request') }}">
                                ¿Olvidaste tu contraseña?
                            </a>
                        @endif

                        <button type="submit" class="bg-[#3cb0a4] hover:bg-[#349b90] text-white rounded-full px-9 py-2.5 text-sm font-bold transition-all shadow-lg active:scale-95">
                            Iniciar sesión
                        </button>
                    </div>
                </form>
