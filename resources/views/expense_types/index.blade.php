...
<div id="expense-type-status-modal" class="hidden fixed inset-0 z-50 items-center justify-center bg-slate-900/60 dark:bg-black/80 backdrop-blur-sm p-4">
    <div id="expense-type-status-card" class="w-full max-w-md bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-2xl transform scale-95 transition-all duration-200">
        <div class="p-6 text-center">
            {{-- ICONO --}}
            <div id="expense-type-status-icon" class="w-14 h-14 rounded-full border-2 border-amber-400 flex items-center justify-center mx-auto text-amber-500 mb-5">
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    document.querySelectorAll('.edit-expense-type-btn').forEach(function (button) {
        button.addEventListener('click', function () {
            ...
            document.getElementById('expense-type-form-title').textContent = 'Editar tipo de gasto';
            document.getElementById('expense-type-submit-btn').textContent = 'Actualizar';
            document.getElementById('expense-type-name').value = name;
            document.getElementById('expense-type-description').value = description;
            document.getElementById('expense-type-active').checked = active;
            document.getElementById('expense-type-form').action = '/expense-types/' + id;
            document.getElementById('expense-type-method').value = 'PUT';
...
function openExpenseTypeModal(type) {
    if (type === 'create') {
        document.getElementById('expense-type-form-title').textContent = 'Nuevo tipo de gasto';
        document.getElementById('expense-type-submit-btn').textContent = 'Guardar';
        document.getElementById('expense-type-form').action = "{{ route('expense-types.store') }}";
        document.getElementById('expense-type-method').value = 'POST';
        document.getElementById('expense-type-name').value = '';
        document.getElementById('expense-type-description').value = '';
        document.getElementById('expense-type-active').checked = true;
...
function openExpenseTypeFormModal() {
    const modal = document.getElementById('expense-type-form-modal');
    const card = document.getElementById('expense-type-form-card');
...
function openExpenseTypeStatusModal(id, name, reactivate) {
    const modal = document.getElementById('expense-type-status-modal');
    const card = document.getElementById('expense-type-status-card');
    const title = document.getElementById('expense-type-status-title');
    const description = document.getElementById('expense-type-status-description');
    const submit = document.getElementById('expense-type-status-submit');
    const icon = document.getElementById('expense-type-status-icon');
    const form = document.getElementById('expense-type-status-form');
    ...
    if (reactivate) {
        ...
        // REACTIVAR
        // =========================================
        title.textContent = '¿Reactivar tipo de gasto?';
        description.textContent = "El tipo de gasto '" + name + "' volverá a estar activo.";
        submit.textContent = 'Sí, reactivar';

        submit.className = 'px-5 py-2.5 rounded-xl bg-emerald-600 hover:bg-emerald-700 text-white text-sm font-bold transition-all';
        icon.className = 'w-14 h-14 rounded-full border-2 border-emerald-400 flex items-center justify-center mx-auto text-emerald-500 mb-5';
        icon.innerHTML = `
            <svg
                class="w-7 h-7"
                fill="none"
        ...
        // INACTIVAR
        // =========================================
        title.textContent = '¿Inactivar tipo de gasto?';
        description.textContent = "El tipo de gasto '" + name + "' pasará a estar inactivo.";
        submit.textContent = 'Sí, inactivar';
        submit.className = 'px-5 py-2.5 rounded-xl bg-rose-600 hover:bg-rose-700 text-white text-sm font-bold transition-all';
        icon.className = 'w-14 h-14 rounded-full border-2 border-amber-400 flex items-center justify-center mx-auto text-amber-500 mb-5';
