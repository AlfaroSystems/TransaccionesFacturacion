...
<div class="animate-fade-in duration-300">
    ...
    <header class="mb-8 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div>
            ...
            <p class="text-slate-400 text-sm font-semibold mt-1">
                Actualiza la información registrada del proveedor y sus contactos.
            </p>
        </div>
    ...
    <div class="bg-white rounded-3xl shadow-sm border border-slate-100 p-8">
        <form action="{{ route('suppliers.update', $supplier) }}" method="POST" class="space-y-8" id="supplier-form">
            @csrf
            @method('PUT')
            ...
            <section>
                ...
                <div id="contacts-container" class="space-y-4">
                    @php
                        $defaultContacts = $supplier->contacts->map(function ($contact) {
                            return [
                        ...
                        if (empty($defaultContacts)) {
                            $defaultContacts = [['full_name' => '', 'phone' => '', 'email' => '']];
                        }

                        $oldContacts = old('contacts', $defaultContacts);
                    @endphp
