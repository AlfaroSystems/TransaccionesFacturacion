                                ...
                                @can('units.desactivar')
                                    @if($unit->is_active)
                                        <button type="button"
                                                ...
                                                class="p-2.5 rounded-xl bg-rose-50 text-rose-600 hover:bg-rose-100 font-semibold text-xs transition-all flex items-center justify-center"
                                    ...
                                    @else
                                        <button type="button"
                                                ...
                                                class="p-2.5 rounded-xl bg-emerald-50 text-emerald-600 hover:bg-emerald-100 font-semibold text-xs transition-all flex items-center justify-center"
...
<div id="create-unit-modal" class="hidden fixed inset-0 z-50 items-center justify-center bg-slate-900/60 backdrop-blur-sm p-4 overflow-y-auto"
    ...
    <div class="relative w-full max-w-2xl bg-white rounded-2xl shadow-2xl p-8 transform scale-95 transition-all">
            ...
            <form action="{{ route('units.store') }}" method="POST" class="space-y-5">
                ...
                {{-- Tipo --}}
                <div>
                    <label for="unit-type" class="block text-xs font-bold text-slate-400 uppercase tracking-wider mb-2">
                        Tipo
                    </label>
...
<div id="edit-unit-modal"
    ...
    <div class="relative w-full max-w-2xl bg-white rounded-2xl shadow-2xl p-8 transform scale-95 transition-all">
            ...
            {{-- FORMULARIO --}}
            <form id="edit-unit-form" action="" method="POST" class="space-y-5">
                @csrf
                @method('PUT')
                <input type="hidden" name="modal_type" value="edit">
                <input type="hidden" name="id" id="edit-unit-id" value="{{ old('modal_type') === 'edit' ? old('id') : '' }}">
                {{-- Nombre --}}
                <div>
