                    ...
                    <div class="space-y-2 bg-slate-50 p-3.5 rounded-xl border border-slate-200">
                        @foreach($roles as $role)
                            <label class="flex items-center gap-2 cursor-pointer">
                                <input type="checkbox" name="roles[]" value="{{ $role->id }}" {{ (is_array(old('roles')) && in_array($role->id, old('roles'))) ? 'checked' : '' }} class="rounded text-navy-sidebar focus:ring-navy-sidebar border-slate-300 w-4 h-4">
