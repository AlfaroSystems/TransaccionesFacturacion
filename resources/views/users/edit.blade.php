...
<div class="max-w-2xl mx-auto animate-fade-in duration-300">
    ...
    <header class="mb-8">
        ...
        <h1 class="text-2xl md:text-3xl font-extrabold text-navy-800 tracking-tight">Editar Usuario</h1>
        <p class="text-slate-400 text-sm font-semibold mt-1">Actualiza los datos de la cuenta de <strong>{{ $user->name }}</strong>.</p>
    </header>
    ...
    <section class="bg-white p-8 rounded-2xl border border-slate-100 card-shadow">
        <form action="{{ route('users.update', $user) }}" method="POST" class="space-y-6">
            ...
            <div>
                ...
                <input type="text" name="name" id="name" value="{{ old('name', $user->name) }}" placeholder="Ej. Juan Pérez" class="w-full bg-slate-50 border @error('name') border-rose-300 focus:border-rose-500 @else border-slate-200 focus:border-navy-sidebar @enderror rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:bg-white transition-all text-slate-700 font-semibold" required>
                @error('name')
            ...
            <div>
                ...
                <input type="email" name="email" id="email" value="{{ old('email', $user->email) }}" placeholder="correo@ejemplo.com" class="w-full bg-slate-50 border @error('email') border-rose-300 focus:border-rose-500 @else border-slate-200 focus:border-navy-sidebar @enderror rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:bg-white transition-all text-slate-700 font-semibold" required>
                @error('email')
                    <p class="text-xs text-rose-500 font-bold mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="border-t border-slate-100 pt-6">
                <h3 class="text-sm font-bold text-navy-800 mb-1">Cambiar Contraseña (Opcional)</h3>
                <p class="text-slate-400 text-xs font-semibold mb-4">Deja estos campos vacíos si no deseas modificar la contraseña actual.</p>

                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <!-- Contraseña -->
                    <div>
                        <label for="password" class="block text-xs font-bold text-slate-400 uppercase tracking-wider mb-2">Nueva Contraseña</label>
                    ...
                    <div>
                        <label for="password_confirmation" class="block text-xs font-bold text-slate-400 uppercase tracking-wider mb-2">Confirmar Nueva Contraseña</label>
                        <input type="password" name="password_confirmation" id="password_confirmation" placeholder="Repite la contraseña" class="w-full bg-slate-50 border border-slate-200 focus:border-navy-sidebar rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:bg-white transition-all text-slate-700 font-semibold">
                    </div>
                </div>
            </div>
